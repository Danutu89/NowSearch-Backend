...
def compare_urls(url_a, url_b):
    ...
    if (
        host_a != host_b
        or url_a.query != url_b.query
        or url_a.fragment != url_b.fragment
    ):
        return False
...
def merge_two_infoboxes(infobox1, infobox2):
    ...
    if "attributes" in infobox2:
        ...
        for attribute in infobox2.get("attributes", []):
            if (
                attribute.get("label") not in attributeSet
...
class ResultContainer:
    ...
    def _merge_infobox(self, infobox):
        ...
        if infobox_id is not None:
            ...
            for existingIndex in self.infoboxes:
                if compare_urls(
                    urlparse(existingIndex.get("id", "")), parsed_url_infobox_id
                ):
    ...
    def __merge_url_result(self, result, position):
        ...
        if result.get("content"):
            result["content"] = WHITESPACE_REGEX.sub(" ", result["content"][:120])
    ...
    def __find_duplicated_http_result(self, result):
        ...
        for merged_result in self._merged_results:
            if "parsed_url" not in merged_result:
                continue
            if compare_urls(
                result["parsed_url"], merged_result["parsed_url"]
    ...
    def __merge_duplicated_http_result(self, duplicated, result, position):
        ...
        if result_content_len(result.get("content", "")) > result_content_len(
            duplicated.get("content", "")
        ):
        ...
        if (
            ...
            and result["parsed_url"].scheme == "https"
        ):
    ...
    def order_results(self):
        ...
        for res in results:
            ...
            current = (
                None
                if category not in categoryPositions
