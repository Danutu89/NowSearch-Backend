...
if not werkzeug_reloader or (
    werkzeug_reloader and os.environ.get("WERKZEUG_RUN_MAIN") == "true"
):
...
def proxify(url):
    ...
    if settings["result_proxy"].get("key"):
        url_params["mortyhash"] = hmac.new(
            settings["result_proxy"]["key"], url.encode(), hashlib.sha256
        ).hexdigest()
...
def render(template_name, override_theme=None, **kwargs):
    ...
    kwargs["opensearch_url"] = (
        ...
        + "?"
        + urlencode(
            {"method": kwargs["method"], "autocomplete": kwargs["autocomplete"]}
        )
    )
...
def _get_ordered_categories():
    ...
    if "categories_order" not in settings["ui"]:
        ...
        ordered_categories.extend(
            x for x in sorted(categories.keys()) if x != "general"
        )
        return ordered_categories
    ordered_categories = settings["ui"]["categories_order"]
    ordered_categories.extend(
        x for x in sorted(categories.keys()) if x not in ordered_categories
    )
    return ordered_categories
...
def pre_request():
    ...
    if not preferences.get_value("language"):
        preferences.parse_dict(
            {"language": _get_browser_or_settings_language(request, LANGUAGE_CODES)}
        )
...
def index_error(output_format, error_message):
    if output_format == "json":
        return Response(
            json.dumps({"error": error_message}), mimetype="application/json"
        )
...
def search():
    ...
    if not request.form.get("q"):
        return index_error("json", "No query"), 400
    ...
    try:
        ...
        return index_error("json", e.message), 400
    except Exception as e:
        logger.exception("search error")
        return index_error("json", gettext("search error")), 500
...
def autocompleter():
    ...
    if (
        ...
        and completer
        and (
            len(raw_text_query.query_parts) > 1
            or (len(raw_text_query.languages) == 0 and not raw_text_query.specific)
        )
    ):
...
def stats_errors():
    ...
    for engine_name in engine_names:
        ...
        sorted_context_count_list = sorted(
            error_stats.items(), key=lambda context_count: context_count[1]
        )
        r = []
        percentage_sum = 0
...
def opensearch():
    ...
    resp = Response(
        response=ret, status=200, mimetype="application/opensearchdescription+xml"
    )
    return resp
...
def favicon():
    return send_from_directory(
        os.path.join(
            app.root_path, static_path, "themes", get_current_theme_name(), "img"
        ),
        "favicon.png",
    ...
    return jsonify(
        {
            ...
            "brand": {"GIT_URL": brand.GIT_URL, "DOCS_URL": brand.DOCS_URL},
